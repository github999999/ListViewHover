...
import xzh.com.listviewhover.view.wheel.WheelView;
        ...
        if (mode != MONTH_DAY) {
            ...
            yearView.setOnWheelViewListener(new WheelView.OnWheelViewListener() {
                ...
                public void onSelected(boolean isUserScroll, int selectedIndex, String item) {
                    ...
                    //需要根据年份及月份动态计算天数
                    days.clear();
                    int maxDays = DateUtil.calculateDaysInMonth(stringToYearMonthDay(item), stringToYearMonthDay(months.get(selectedMonthIndex)));
        ...
        monthView.setOnWheelViewListener(new WheelView.OnWheelViewListener() {
            ...
            public void onSelected(boolean isUserScroll, int selectedIndex, String item) {
                ...
                if (mode != YEAR_MONTH) {
                    ...
                    days.clear();
                    int maxDays = DateUtil.calculateDaysInMonth(stringToYearMonthDay(years.get(selectedYearIndex)), stringToYearMonthDay(item));
                    for (int i = 1; i <= maxDays; i++) {
                        days.add(DateUtil.fillZero(i));
                    }
                    if (selectedDayIndex >= maxDays) {
